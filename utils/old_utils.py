...
def basic_check(name, seed):
    ...
    for episode in range(no_episodes):
        ...
        while True:
            env.render()
            a_t = np.random.uniform(-1, 1, env.action_space_size)
            o_t_1, r_t, done = env.step(a_t)
            ep_r += r_t
            t += 1
            o_t = o_t_1
            if done:
                print("Episode finished with total reward ",
                      ep_r, "time steps", t)
                break
...
def energy_check(name, seed):
    ...
    while True:
        ...
        # env.render()
        a_t = np.zeros(env.action_space_size)
        o_t_1, r_t, done = env.step(a_t)
        ep_r += r_t
        t += 1
        o_t = o_t_1
    ...
    path = './environments/energy_check'
