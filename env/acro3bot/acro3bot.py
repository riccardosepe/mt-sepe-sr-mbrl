...
class acro3bot(BaseEnv):
    ...
    def get_reward(self):
        upright = (np.array([np.cos(self.state[0]), np.cos(
            self.state[0]+self.state[1]), np.cos(self.state[0]+self.state[1]+self.state[2])])+1)/2
    ...
    def draw(self):
        ...
        link2_points = rect_points(
            link2_center, self.l1, height, np.pi/2-self.state[0], scaling, offset)
        ...
        joint3 = [offset[0]+scaling*((self.l1)*np.sin(self.state[0])),
        ...
        link3_points = rect_points(
            link3_center, self.l2, height, np.pi/2-self.state[0]-self.state[1], scaling, offset)
        pygame.draw.polygon(self.screen, (72, 209, 204),
        ...
        link4_center = np.array([(self.l1)*np.sin(self.state[0])+(self.l2)*np.sin(self.state[0]+self.state[1])
                                 ...
                                 (self.l1)*np.cos(self.state[0])+(self.l2) *
                                 np.cos(self.state[0]+self.state[1])
                                 + (self.l3/2)*np.cos(self.state[0]+self.state[1]+self.state[2])])
        link4_points = rect_points(link4_center, self.l3, height, np.pi /
                                   2-self.state[0]-self.state[1]-self.state[2], scaling, offset)
        pygame.draw.polygon(self.screen, (72, 209, 204),
                            link4_points)  # medium turquoise
        pygame.draw.circle(self.screen, (255, 69, 0), joint4,
                           scaling*height/1.8)  # orange red
        ...
        end_point = [offset[0]+scaling*end_point_plot[0],
                     offset[1]-scaling*end_point_plot[1]]
