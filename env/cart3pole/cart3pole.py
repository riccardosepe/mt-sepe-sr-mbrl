...
class cart3pole(BaseEnv):
    ...
    def get_reward(self):
        upright = (np.array([np.cos(self.state[1]), np.cos(
            self.state[1]+self.state[2]), np.cos(self.state[1]+self.state[2]+self.state[3])])+1)/2
        ...
        ang_vel = np.array(
            [qdot[0], qdot[1], qdot[1]+qdot[2], qdot[1]+qdot[2]+qdot[3]])
    ...
    def draw(self):
        ...
        link4_center = np.array([plot_x+(self.l2)*np.sin(self.state[1])+(self.l3)*np.sin(self.state[1]+self.state[2])
                                 ...
                                 (self.l2)*np.cos(self.state[1])+(self.l3) *
                                 np.cos(self.state[1]+self.state[2])
                                 + (self.l4/2)*np.cos(self.state[1]+self.state[2]+self.state[3])])
        link4_points = rect_points(link4_center, self.l4, height, np.pi /
                                   2-self.state[1]-self.state[2]-self.state[3], scaling, offset)
        pygame.draw.polygon(self.screen, (72, 209, 204),
        ...
        end_point_plot = np.array([plot_x+(self.l2)*np.sin(self.state[1])+(self.l3)*np.sin(self.state[1]+self.state[2])
                                   ...
                                   (self.l2)*np.cos(self.state[1])+(
                                       self.l3)*np.cos(self.state[1]+self.state[2])
