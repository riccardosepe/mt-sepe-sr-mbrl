...
class pendulum(BaseEnv):
    ...
    def draw(self):
        ...
        link1_points = rect_points(
            link1_center, self.l1, height/1.8, np.pi/2-self.state[0], scaling, offset)
        pygame.draw.polygon(self.screen, (72, 209, 204),
                            link1_points)  # medium turquoise
        pygame.draw.circle(self.screen, (255, 69, 0), joint1,
                           scaling*height/1.25)  # orange red

        end_point = np.array(
            [(self.l1)*np.sin(self.state[0]), (self.l1)*np.cos(self.state[0])])
        end_point_plot = [offset[0]+scaling *
                          end_point[0], offset[1]-scaling*end_point[1]]
        pygame.draw.circle(self.screen, 'slateblue1',
                           end_point_plot, scaling*0.15)  # orange red #endpoint
