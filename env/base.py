...
class BaseEnv:
    def __init__(self, name, n, obs_size, action_size, inertials, dt, a_scale, mle):
        ...
        # work done by non conservative forces (motor torques)
        self.w = np.array([0.0])
    ...
    def rk4(self, s_all, da_ds):
        ...
        for l in range(4):
            ...
            for m in range(self.phi_dim):
                ...
                phidot[:, m] = (np.matmul(jac_s, phi[:, m]) +
                                np.matmul(jac_a, da[:, m]))
            k_l = np.concatenate(
                (sdot, self.a_zeros, self.get_power(a, sdot), phidot.flatten()))
            k.append(k_l)
            if l == 0 or l == 1:
    ...
    def step(self, a, da_ds=None):
        ...
        if self.mle:
            ...
            ns = self.rk4(s_all, np.array(
                [da_ds[j]*self.a_scale[j] for j in range(self.action_size)]))
            ns, nw, nphi = ns[:-(self.phi_len+1+self.action_size)], ns[-(
                self.phi_len+1):-self.phi_len], ns[-self.phi_len:]  # omit action
