...
class ModelEnv(BaseEnv):
    ...
    @property
    def eps(self):
        if np.abs(self._state[0, 0]) < self._eps:
            eps = self._eps * np.sign(self._state[0, 0])
        else:
            eps = 0
    ...
    def reset(self):
        ...
        state = np.concatenate((initial_pos, np.zeros(self.n, )))[:, None]
